...
const SE_RE = /(?:^|[^A-Za-z0-9]|(?<=[a-z]))S(?<season>\d{1,4})(?:[\s._-]?(?:Part|Pt|P)[\s._-]?(?<part>\d{1,2}))?[\s._-]?E(?<episode>\d{1,4}(?:[\s._-]?E\d{1,4})*)\b/i;
const SEASON_ONLY_RE = /\bSeason[\s._-]?(\d{1,4})\b/i;
const EPISODE_ONLY_RE = /\b(?:Episode|EP|Ep)[\s._-]?(\d{1,4})\b/i;
