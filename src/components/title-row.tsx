...
export function TitleRow({ title, items, loading, emptyHint }: Props) {
  ...
  return (
    <section className="py-6">
      <h2 className="px-4 md:px-6 mb-4 text-xl md:text-2xl font-display font-bold tracking-tight">
        {title}
      </h2>
      {showEmpty ? (
        ...
        <div className="scrollbar-hide overflow-x-auto">
          <div className="flex gap-4 px-4 md:px-6 pb-3">
            {loading
              ...
              : items!.map((item) => (
                  <div key={item.id} className="w-[160px] md:w-[200px] shrink-0">
                    <TitleCard item={item} />
                  </div>
                ))}
          </div>
